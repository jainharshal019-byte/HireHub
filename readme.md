...
HireHub is a job listing website from my [PHP From Scratch course](https://www.traversymedia.com/php-from-scratch). It includes a custom Laravel-like router, controller classes, views, a database layer and a project structure using namespaces and PSR-4 autoloading. It highlights how to structure a PHP project without using any frameworks or libraries.

![HireHub](/public/images/screen.jpg)
...
2. Create a database called `hirehub`
3. Import the `hirehub.sql` file into your database
...
```conf
DocumentRoot "C:/xampp/htdocs/hirehub/public"
<Directory "C:/xampp/htdocs/hirehub/public">
```
...
```conf
DocumentRoot "/Applications/MAMP/htdocs/hirehub/public"
<Directory "/Applications/MAMP/htdocs/hirehub/public">
```

##### Laragon

If you are using Laragon, you can set right-click the icon in the system tray and go to `Apache > sites-enabled > auto.hirehub.test.conf`. Your file may be named differently.

You can then set the document root. Here is an example:

```conf
<VirtualHost *:80>
    DocumentRoot "C:/laragon/www/hirehub/public"
    ServerName hirehub.test
    ServerAlias *.hirehub.test
    <Directory "C:/laragon/www/hirehub/public">
        AllowOverride All
        Require all granted
    </Directory>
</VirtualHost>
...
- **hirehub.sql** - Contains the database dump
